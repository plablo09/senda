...
async def test_first_error_always_gives_feedback():
    ...
    with patch("api.services.feedback_rate_limiter.aioredis.from_url", return_value=redis_mock):
...
async def test_silence_window_blocks_feedback():
    ...
    assert decision.limite is False
    redis_mock.hincrby.assert_called_once_with("feedback:session-1:ejercicio-1", "attempts_since_feedback", 1)
